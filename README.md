...
- **Pseudonymize** staging/QA exports — deterministic (seeded) or run-specific (non-seeded) field transformation; PII fields identified and modeled manually in the XML pipeline
...
| Deterministic seeding in the DSL (`<setup rngSeed>`) | ✅ entities, generators and script expressions | — |
...
| CLI + local execution | ✅ | ✅ |
| **Scale** | Python multiprocessing (and optional Ray) | Enterprise execution controls and optimisations |
...
|---|---|---|
| **Performance** | Throughput-oriented execution | Throughput-oriented execution |
...
Developers and data engineers who need deterministic synthetic data generation or PII-aware pseudonymization in local environments, CI pipelines, or agent-driven workflows. PII field identification is manual in CE; the EE DataWorkbench assists review and users confirm mappings.
...
- Set `<setup rngSeed="N">` and seeded generation replays on the supported runtime, including generator, entity, and script-expression paths. Seeded date-derived values use the deterministic clock anchor; without a seed, generation is not replay-stable.
...
request = {
    ...
    "version": "v1",
    "count": 1,
    "seed": "regression-suite-42",       # same seed → replayable on the supported runtime
    "locale": "en_US",
...
# Same seeded Random → same CreditCard fields on the supported runtime.
...
### Determinism evidence — CE; EE scope not established here

| Scope | CE | Enterprise Platform* |
|---|---|---|
| **Facade** (`generate_domain` registered domains) | ✅ replay-identical, CI-gated (same runtime) | — |
| **Domain services** (direct use with seeded `rng=...`) | ✅ replay-identical, CI-gated (same runtime) | — |
| **Literal generators** | ✅ replay-identical (same runtime) | — |
| **RNG / clock runtime SPOTs** | ✅ `spawn_rng`, `now_utc_naive`, `resolve_clock` | — |
| **Architecture gates in CI** | ✅ facade replay + service replay (every service) + clock drift | — |
| **Custom XML pipelines** (seeded via `<setup rngSeed>`) | ✅ reproducible on the same runtime (single-process) | — |
| **Multi-system coordinated execution** (Oracle + MongoDB + Kafka in one run) | — | — |
| **Seeded vs unseeded pseudonymization** | ✅ manual model | ✅ scanner-assisted, user-confirmed model |
| **Threat-led / TLPT-grade audit evidence** (full contract enforcement, per-stage execution logging) | — | — |

\* EE behavior in this table is outside this repository's evidence.
...
|---|---|---|---|
| **Seeded** (`rngSeed` set) | Deterministic on the supported runtime | Can support pseudonymisation (GDPR Art. 4(5)) if the seed is kept separately and access-controlled | Regression testing, stable CI/CD pipelines |
...
<setup defaultSeparator="," rngSeed="42">
  <generate name="customers" source="customer_export.csv" target="CSV" distribution="ordered">
    <!-- rngSeed makes seeded runs produce the same pseudonyms on the supported runtime; remove it for a new
...
so the same value gives the same token on the supported runtime; without a seed each run gets
a new random key:
...
With `rngSeed`: the same source record in the same ordered row position and descriptor gives the same pseudonym on the supported runtime, for regression testing.
...
| **GDPR** — Art. 4(5) pseudonymization definition; Art. 25 privacy by design; Art. 32 security of processing | Seeded pseudonymization with repeatable mapping when the seed is retained securely; non-seeded mode is not replay-stable |
| **PCI DSS 4.0** — Req. 6.5.5 (live PANs prohibited in test/development) | Synthetic PAN generation for test environments; seeded tokenisation is repeatable on the supported runtime |
...
CE and EE share the DATAMIMIC DSL. The execution layer is separate: CE is a Python execution engine using multiprocessing (with optional Ray for distribution); EE is an independently-optimised execution engine with a Rust fastpath, ML/auto-regressive generation, and keyset and manifest building from live schemas.
...
║  │  Optimised distributed execution                         │    ║
