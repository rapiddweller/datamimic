...
class CityDataLoader(BaseDataLoader):
    ...
    #     """
    #     if self._processed_state_data_dataset is None:
    #         processed_dataset_list = self._get_processed_dataset_list(self._state_data_dataset_not_found_set)
    #         for dataset in processed_dataset_list:
    #             try:
    #                 file_path = Path(__file__).parent.parent.parent.parent / "domain_data" / "common" / "state" / f"state_{dataset}.csv"
    #                 return_value = FileContentStorage.load_file_with_custom_func(cache_key=str(file_path), read_func=lambda: FileUtil.read_csv_to_dict_of_tuples_with_header(file_path, delimiter=";"))
    #                 self._processed_state_data_dataset = dataset
    #                 return return_value
    #             except FileNotFoundError as e:
    #                 self._state_data_dataset_not_found_set.add(dataset)  
    #                 logger.warning(f"Error loading dataset '{dataset}': {e}")
    #         raise ValueError(f"No state data found for '{self._country_code}'")
    #     else:
    #         file_path = Path(__file__).parent.parent.parent.parent / "domain_data" / "common" / "state" / f"{self._processed_state_data_dataset}.csv"
    #         return FileContentStorage.load_file_with_cache(str(file_path))
    ...
    def get_random_city(self) -> dict[str, Any]:
        ...
        return {
            ...
            # TODO: Add state name
            # "state": city_row[city_header_dict.get("state")],
