...
class Bank(BaseEntity):
    ...
    @property_cache
    def bic(self) -> str:
        return StringGenerator.rnd_str_from_regex("[A-Z]{4}DE[A-Z0-9]{2}")
    ...
    def customer_service_phone(self) -> str:
        ...
        if hasattr(self._bank_generator, "dataset") and self._bank_generator.dataset == "de_DE":
            # German format
            return "+49 " + StringGenerator.rnd_str_from_regex("[0-9]{3} [0-9]{5,7}")
        else:
            # US format
            return "+1 " + StringGenerator.rnd_str_from_regex("[0-9]{3}-[0-9]{3}-[0-9]{4}")
