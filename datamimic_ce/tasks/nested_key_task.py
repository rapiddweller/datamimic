...
class NestedKeyTask(Task):
        ...
        if self._sub_tasks:
            for sub_task in self._sub_tasks:
                try:
                    ...
                    elif isinstance(sub_task, SourceConstraintsTask):
                        # do not execute SourceConstraintsTask here, SourceConstraintsTask is for filter source data
                        pass
                    else:
    ...
    def _filter_source_by_constraints_task(self, parent_context: GenIterContext, source_data: list) -> list:
        """
        Execute SourceConstraintsTask to filter source data
        """
        result = source_data
        if self._sub_tasks:
            for sub_task in self._sub_tasks:
                if isinstance(sub_task, SourceConstraintsTask):
                    nestedkey_len = self._determine_nestedkey_length(context=parent_context)
