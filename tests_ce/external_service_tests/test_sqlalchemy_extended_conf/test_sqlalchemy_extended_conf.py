...
class TestSqlAlchemyExtendedConf:
    ...
    def test_none_as_null(self):
        ...
        env_file = (
            f"conf/{'local' if os.getenv('RUNTIME_ENVIRONMENT') == 'development' else 'environment'}.env.properties"
        )
        ...
        data_null = connection.execute(
            sqlalchemy.text(f"SELECT * FROM {schema}.none_as_null WHERE data_null IS NULL")
        ).fetchall()
        # Fetch records where data_none is SQL null (0 records)
        false_data_none = connection.execute(
            sqlalchemy.text(f"SELECT * FROM {schema}.none_as_null WHERE data_none IS NULL")
        ).fetchall()
        # Fetch records where data_none is 'null' (5 records)
        true_data_none = connection.execute(
            sqlalchemy.text(f"SELECT * FROM {schema}.none_as_null WHERE data_none = 'null'")
        ).fetchall()
