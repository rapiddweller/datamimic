...
(``all_entities_seeded.xml``): every registered domain entity is generated with a
per-variable ``rngSeed`` and a handful of scalar fields, plus one scalar reached
through every reliably accessible sub-structure (``e.address.street``). Running
the same model twice must yield byte-identical output per ``<generate>`` block.

The model is GENERATED from the entity registry (the shared
``dsl_model_builder.build_all_entities_seeded_xml``) and the generated text is
committed so a reviewer can read it top-to-bottom. A sync-check test fails if the
committed file drifts from the registry, so a newly added entity must be
regenerated in — it cannot silently skip the gate.
