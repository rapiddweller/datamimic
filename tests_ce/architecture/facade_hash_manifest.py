"""Generate and compare the cross-platform facade hash manifest."""
...
class FacadeHashManifest(TypedDict):
...
def build_actual_manifest() -> FacadeHashManifest:
    ...
    return {"facade_hashes": hashes, "utf8_probe_hash": probe}
...
def read_manifest(path: Path) -> FacadeHashManifest:
...
def compare_manifests(manifests: Mapping[str, FacadeHashManifest]) -> tuple[str, ...]:
    errors: list[str] = []
    if not manifests:
        return ("no facade hash manifests found",)
...
def _write_summary(manifests: Mapping[str, FacadeHashManifest], errors: tuple[str, ...]) -> None:
    summary_path = os.environ.get("GITHUB_STEP_SUMMARY")
    if summary_path is None:
        return
    status = "PASS" if not errors else "FAIL"
    headers = [*EXPECTED_CONTENT_HASHES, "utf8_probe"]
    lines = [
        "## Facade golden hash fan-in",
        "",
    ...
    golden_values = [*EXPECTED_CONTENT_HASHES.values(), UTF8_PROBE_HASH]
...
def _generate_command(output: Path) -> int:
    ...
    print(f"Wrote facade hash manifest: {output}")
    return 0


def _compare_command(root: Path, expected_count: int) -> int:
    paths = sorted(root.rglob("facade-hash-manifest.json"))
    ...
    print(f"Compared {len(manifests)} actual facade hash manifests directly.")
    for label, manifest in manifests.items():
    ...
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
    print("All actual hashes match each other and the committed goldens.")
    return 0
