...
def _recipe_xml(recipe_id: str) -> str:
    return (resources.files("datamimic_ce.authoring") / "recipes" / f"{recipe_id}.xml").read_text(encoding="utf-8")
...
def test_gate3_no_ee_terms_in_agent_content() -> None:
    corpus = "".join(
        ...
        for topic in (
            ...
            ReferenceTopic.TARGETS,
            ReferenceTopic.DISTRIBUTIONS,
            ReferenceTopic.CONTEXT,
            ReferenceTopic.TIMESERIES,
            ReferenceTopic.RECIPES,
        )
    )
...
def test_reference_distributions_documents_numeric_sequences() -> None:
    ...
    text = reference(ReferenceTopic.DISTRIBUTIONS)
...
def test_reference_generators_derived_and_reserved_names_covered() -> None:
    ...
    assert "IncrementGenerator" in reference(ReferenceTopic.GENERATORS, "increment")
...
def test_reference_entities_enumerated_from_registry() -> None:
    ...
    listing = reference(ReferenceTopic.ENTITIES)
    for entity in (s.entity for s in list_entity_specs()):
        assert entity in listing
    person = reference(ReferenceTopic.ENTITIES, "Person")
    assert "given_name" in person and "email" in person
    with pytest.raises(ValueError, match="Unknown entity"):
        reference(ReferenceTopic.ENTITIES, "NotAnEntity")
...
def test_element_model_map_covers_all_element_constants() -> None:
    ...
    declared = {
        value for name, value in vars(element_constants).items() if name.startswith("EL_") and isinstance(value, str)
    }
...
def test_constraint_three_surface_consistency_all_or_none() -> None:
    ...
    all_or_none_a = next((c for c in constraints_a if c.get("kind") == "all_or_none"), None)
    ...
    all_or_none_b = next((c for c in constraints_b if c.get("kind") == "all_or_none"), None)
    ...
    assert attrs_a == attrs_b == attrs_c, (
        f"AllOrNone attrs mismatch across surfaces: json_schema={attrs_a}, manifest={attrs_b}, prose={attrs_c}"
    )
