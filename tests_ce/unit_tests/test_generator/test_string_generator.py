...
class TestStringGenerator:
    ...
    def test_string_generator_valid(self, min_length: int, max_length: int, char_set: str, prefix: str, suffix: str,
                                    unique: bool):
        if not char_set:
            char_set = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
        print(char_set)
        generator = StringGenerator(self.get_class_factory_util(), min_len=min_length, max_len=max_length,
                                    char_set=char_set, unique=unique, prefix=prefix, suffix=suffix)
        for _ in range(10):
    ...
    def test_string_generator_invalid(self):
        with pytest.raises(ValueError, match=re.escape(
                "Failed when init StringGenerator because min_len(9) > max_len(4)")):
    ...
    def test_string_generator_unique_true_limit_charset(self):
        with pytest.raises(ValueError,
                           match=re.escape("Cannot generate unique string with length 9 from character set of size 5")):
            StringGenerator(self.get_class_factory_util(), char_set="[a-b]", min_len=5, max_len=9, unique=True).generate()
