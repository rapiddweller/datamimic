...
class TestRAMMysqlRead:
    ...
    def _insert_mysql_data(self, filename: str, count: str, multiprocessing: str):
    ...
    def test_single_thread_mysql_read_100(self, class_fixture) -> None:
        count = "100"
        multiprocessing = "False"
        test_group = "single_thread_mysql_read"
        test_case = f"test_{count}"
        rounds = 2
        # insert data
        self._insert_mysql_data("ram_measure_mysql_write.xml", count=count, multiprocessing=multiprocessing)
        # execute measure
        time.sleep(1)  # Warm-up period
        xml_path = self._test_dir.joinpath("ram_measure_mysql_read.xml")
    ...
    def test_multiprocessing_mysql_read_100(self, class_fixture) -> None:
        ...
        test_group = "multiprocessing_mysql_read"
        test_case = f"test_{count}"
        rounds = 2
        # insert data
        self._insert_mysql_data("ram_measure_mysql_write.xml", count=count, multiprocessing=multiprocessing)
