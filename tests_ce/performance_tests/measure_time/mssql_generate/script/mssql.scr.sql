...
-- Check if 'time_measure' table exists in the 'simple' schema
IF OBJECT_ID(N'simple.time_measure', N'U') IS NULL
BEGIN
    -- Create the "time_measure" table in the 'simple' schema
    CREATE TABLE simple.time_measure (
        ...
        name NVARCHAR(100),
        active BIT NOT NULL,
        CONSTRAINT UQ_time_measure_id UNIQUE (id)
    );
END